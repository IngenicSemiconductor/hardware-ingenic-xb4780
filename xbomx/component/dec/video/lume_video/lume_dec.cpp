...
void VpuMem::vpu_mem_dealloc(void* ptr) {
    ...
    int count = 0;
    while(count < mDevBuffers.size()) {
	sp<MemoryHeapBase> devbuf = mDevBuffers.editItemAt(count);
	if (devbuf.get()->getBase() == ptr) {
	    EL("Found ptr 0x%08x at %d pos in mDevBuffers list.", ptr, count);
	    mDevBuffers.removeItemsAt(count);
	    break;
	}
	count++;
    }
