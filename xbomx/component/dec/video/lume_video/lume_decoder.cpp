...
#define DUMP_DATA 0
    ...
    if(!copy_bs){
      if (*inslen > max_copy_len)
	max_copy_len = *inslen;
      copy_bs=(char*)jz4740_alloc_frame(avctx->VpuMem_ptr,32,max_copy_len);
    }else if (*inslen > max_copy_len){
      max_copy_len = *inslen;
      jz4740_free_frame(avctx->VpuMem_ptr, copy_bs);
      copy_bs=(char*)jz4740_alloc_frame(avctx->VpuMem_ptr,32,max_copy_len);
    }
