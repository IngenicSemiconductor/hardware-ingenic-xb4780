...
status_t HWAudioDec::initDecoder() {
    ...
    if(!aContext) {
      ...
      aContext->samplerate = mSamplingRate;
      aContext->channels = mNumChannels;
...
OMX_ERRORTYPE HWAudioDec::internalSetParameter(
        ...
        case 0x7F000014:
	{
	    ...
	    /*get samplerate and numchannels from ACodec.cpp*/
	    if (aacParams->nSampleRate) mSamplingRate = aacParams->nSampleRate; 
	    if (aacParams->nChannels) mNumChannels = aacParams->nChannels;
	    ...
	    /*get samplerate and numchannels from ACodec.cpp*/
	    if (pcmParams->nSamplingRate) mSamplingRate = pcmParams->nSamplingRate;
	    if (pcmParams->nChannels) mNumChannels = pcmParams->nChannels;
