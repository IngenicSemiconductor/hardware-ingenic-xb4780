...
static int out_getbuffer(struct audio_stream_out *stream, struct buffer_info *binfo)
{
	const struct legacy_stream_out *out =
		reinterpret_cast<const struct legacy_stream_out *>(stream);
	return out->legacy_out->ioctrl(AudioStreamOut_GetBuffer, binfo);
}

static int out_setbuffer(struct audio_stream_out *stream, struct buffer_info *binfo)
{
	const struct legacy_stream_out *out =
		reinterpret_cast<const struct legacy_stream_out *>(stream);
	return out->legacy_out->ioctrl(AudioStreamOut_SetBuffer, binfo);
}
                                   ...
                                   struct audio_stream_out **stream_out)
{
	...
	out->stream.getbuffer = out_getbuffer;
	out->stream.setbuffer = out_setbuffer;
