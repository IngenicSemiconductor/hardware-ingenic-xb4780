...
#define STREAMTYPE_PVFILE 22

#define STREAM_BUFFER_SIZE (16*2048)
...
#define STREAM_CTRL_SET_AVSYNC 9
...
typedef struct streaming_control {
	...
	int (*streaming_seek)( int fd, loff_t pos, struct streaming_control *stream_ctrl );
	void *data;
} streaming_ctrl_t;
...
typedef struct stream {
  ...
  // Seek
  int (*seek)(struct stream *s,loff_t pos);
  // Control
...
int stream_seek_long(stream_t *s, loff_t pos);

#ifdef CONFIG_STREAM_CACHE
int stream_enable_cache(stream_t *stream,int size,int min,int prefill);
int cache_stream_fill_buffer(stream_t *s);
int cache_stream_seek_long(stream_t *s,loff_t pos);
#else
...
inline static loff_t stream_tell(stream_t *s){
  return s->pos+s->buf_pos-s->buf_len;
}

inline static int stream_seek(stream_t *s,loff_t pos){

  mp_dbg(MSGT_DEMUX, MSGL_DBG3, "seek to 0x%qX\n",(long long)pos);

  if(pos<s->pos){
    loff_t x=pos-(s->pos-s->buf_len);
    if(x>=0){
      s->buf_pos=x;
...
inline static int stream_skip(stream_t *s,loff_t len){
...
//====add for opencore start
#ifdef __cplusplus
extern "C"
{
#endif
    stream_t* open_stream_opencore(char* filename,char** options, 
				   int* file_format,int fd);
#ifdef __cplusplus
}
#endif
stream_t* open_stream_plugin_opencore(stream_info_t* sinfo,char* filename,
				      int mode, char** options, 
				      int* file_format, int* ret,int fd);
//====add for opencore end
