...
static int fill_buffer(stream_t *s, char *but, int len)
{
  if(s->type == STREAMTYPE_DVD) {
    loff_t pos=dvd_read_sector(s->priv,s->buffer);
    if(pos>=0) {
...
static int open_s(stream_t *stream,int mode, void* opts, int* file_format) {
  ...
  if(1){
    ...
    stream->start_pos = (loff_t)d->cur_pack*2048;
    stream->end_pos = (loff_t)(d->cur_pgc->cell_playback[d->last_cell-1].last_sector)*2048;
