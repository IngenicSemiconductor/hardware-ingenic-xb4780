...
double get_mp3_duration(demuxer_t *demuxer){
    uint8_t hdr[4];
    int len,nf=0,i,framesize,srate;
	...
	break;
      ...
      len = mp_get_mp3_header(hdr, NULL, &srate, &framesize, NULL, NULL);
      if(len < 0) {
    ...
    if(nf && srate && framesize){
      double duration = (double)((double)(nf*framesize)/(double)srate);
      return duration;
    }
