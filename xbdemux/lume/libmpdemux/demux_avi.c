                                               ...
                                               unsigned int id)
{
  ...
  if(stream_id==demux->audio->id){
	...
	priv->audio_block_no = (sh->audio.dwRate)*(priv->avi_audio_pts)/(sh->audio.dwScale);
...
do{
  int flags=AVIIF_KEYFRAME;//hpwang 20111013 change the flag value
  ...
  if(priv->idx_size>0 && priv->idx_pos<priv->idx_size){
    ...
    pos = (loff_t)priv->idx_offset+AVI_IDX_OFFSET(idx);
...
// ------------ STEP 1: find nearest video keyframe chunk ------------
      ...
      if (video_chunk_pos == priv->idx_size-1){ // we find the last frame, but can not seek to there!! //hpwang 2011-12-02
	demux_seek_avi(demuxer, (rel_seek_secs - 1.0), audio_delay, flags); //
	return;
      }
...
// ------------ STEP 2: seek audio, find the right chunk & pos ------------
	...
	if(sh_audio->audio.dwSampleSize && sh_audio->audio.dwSampleSize != 1){
