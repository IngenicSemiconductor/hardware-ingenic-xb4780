...
#define MAX_CHUNK_LEN_PROBE (0x200000)
	  if(len > MAX_CHUNK_LEN_PROBE){
	    LOGE("unnormal ftyp chunk len = %d > MAX_CHUNK_LEN_PROBE = %d", len, MAX_CHUNK_LEN_PROBE);
	    //return 0; ?
	    break;
	  }
...
//	case MOV_FOURCC('c','m','o','v'):
	  ...
	  priv->moov_start=(loff_t)stream_tell(demuxer->stream);
	  priv->moov_end=(loff_t)priv->moov_start+len-skipped;
...
static int lschunks_intrak(demuxer_t* demuxer, int level, unsigned int id,
                           loff_t pos, loff_t len, mov_track_t* trak);
...
static void lschunks(demuxer_t* demuxer,int level,loff_t endpos,mov_track_t* trak){
    mov_priv_t* priv=demuxer->priv;
...
//	    int temp=stream_read_dword(demuxer->stream);
	...
	case MOV_FOURCC('u','d','t','a'):
	{
		...
		switch (udta_id)
		{
		    ...
		    {
			loff_t text_len = stream_read_word(demuxer->stream);
...
*/
    ...
    case MOV_FOURCC('s','t','s','d'): {
      ...
      for (i = 0; i < count; i++) {
        ...
        loff_t len = stream_read_dword(demuxer->stream);
...
//	x*=char2int(trak->stdata,32); // bytes/packet
	...
	if(frame<trak->editlist[trak->editlist_pos].start_frame)
	    trak->editlist_pos=0;
#if 0 /* Some track may have multiple edit lists, which can cause a/v desync,
        so we always use the first edit list.*/
...
if(trak->pos==0 && trak->stream_header_len>0){
    ...
    if (trak) {
      ...
      if (samplenr < 0)
        ...
        loff_t pos = trak->samples[samplenr].pos;
