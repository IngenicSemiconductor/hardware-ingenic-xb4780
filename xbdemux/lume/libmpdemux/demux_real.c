...
static int parse_index_chunk(demuxer_t *demuxer)
{
    ...
    long long next_header_pos = priv->index_chunk_offset;
...
// we will not use audio index if we use -idx and have a video
	if(((!demuxer->video->sh && priv->index_mode == 2) || priv->is_multirate) && (unsigned)demuxer->audio->id < MAX_STREAMS) {
...
//    for (i = 1; ; i++)
	...
	switch(chunk_id)
	{
		...
		if(!priv->duration)
		  priv->duration = stream_read_dword(demuxer->stream)/1000;
		else
		  stream_skip(demuxer->stream, 4); /* duration */
...
static void demux_seek_real(demuxer_t *demuxer, float rel_seek_secs, float audio_delay, int flags)
{
	...
	streams |= 2;
    if(!priv->index_malloc_size[vid])
      demuxer->seekable|=0x2;
//    printf("streams: %d\n", streams);
