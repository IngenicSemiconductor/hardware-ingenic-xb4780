...
//	      printf("### rlen=%d   \n",rlen);
		    ...
		    if(demux->filepos==demux->movi_start)
		      {
			asf->start_time = AV_RL32(p);
		      }
		    if(asf->start_time > 0x80000000)//MAX_PTS
		      time2 = AV_RL32(p);
		    else
		      time2=AV_RL32(p)-asf->start_time; // read PTS
            	    if (asf->asf_is_dvr_ms)
...
//    sh_video_t *sh_video=d_video->sh;
  ...
  //================= seek in ASF ==========================
    ...
    loff_t rel_seek_packs=(flags&SEEK_FACTOR)?	 // FIXME: int may be enough?
	(rel_seek_secs*(demuxer->movi_end-demuxer->movi_start)/asf->packetsize):
	(rel_seek_secs*p_rate);
    loff_t rel_seek_bytes=rel_seek_packs*asf->packetsize;
    loff_t newpos;
