...
static int matroska_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    for (i=0; i < matroska->tracks.nb_elem; i++) {
	    ...
	    int data_len = (long long unsigned int)(matroska->ctx->duration * track->audio.out_samplerate) & 0xFFFFFFFFFFFFFFFF;
	    put_le32(&b, data_len);
	    // LOGE("track->audio.channels=%lld,track->audio.bitdepth=%lld,track->audio.out_samplerate=%f,matroska->ctx->duration=%lld,matroska->ctx->duration * track->audio.out_samplerate = %f\n",track->audio.channels,track->audio.bitdepth,track->audio.out_samplerate,matroska->ctx->duration,matroska->ctx->duration * track->audio.out_samplerate);
