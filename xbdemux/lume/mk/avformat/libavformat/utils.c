...
#else
    int genpts;
    if (!strcmp(s->iformat->name, "mpeg")){
      genpts= 1;
    }else{
      genpts= s->flags & AVFMT_FLAG_GENPTS;
    }   
#endif
    for(;;){
        pktl = s->packet_buffer;
...
int av_seek_frame(AVFormatContext *s, int stream_index, int64_t timestamp, int flags)
{
    ...
    if (ret >= 0) {
      ...
      //if(accurate_seek_try-->0)
      //return -1;      
      /*if(s->thumb_index_unused ){
	ret = s->iformat->read_seek(s, stream_index, 0, flags);
	return 0;
	}*/
    }
...
/*            if(!st->time_base.num)
	...
	//rliu add for h264 timebase. fixme:mpeg2 also need?
        if (st->codec->codec_id == CODEC_ID_H264) {
	  if(st->codec->ticks_per_frame == 1){
            st->codec->time_base.den *=2;
	  }
