...
typedef struct {
  ...
  // filler's pointers:
  int eof;
  loff_t min_filepos; // buffer contain only a part of the file, from min-max pos
  loff_t max_filepos;
  loff_t offset;      // filepos <-> bufferpos  offset value (filepos of the buffer's first byte)
  // reader's pointers:
  loff_t read_filepos;
  // commands/locking:
...
//  int fifo_flag;  // 1 if we should use FIFO to notice cache about buffer reads.
  ...
  volatile loff_t control_new_pos;
...
int cache_stream_seek_long(stream_t *stream,loff_t pos){
  cache_vars_t* s;
  loff_t newpos;
