...
{
  register loff_t a = AVI_IDX_OFFSET((AVIINDEXENTRY *)elem1);
  register loff_t b = AVI_IDX_OFFSET((AVIINDEXENTRY *)elem2);
  return (a > b) - (b > a);
}
...
while(1){
  ...
  switch(id){
	...
	stream_read(demuxer->stream,(char*) &(priv->avih),FFMIN(size2,sizeof(priv->avih)));
	le2me_MainAVIHeader(&(priv->avih)); // swap to machine endian
      chunksize-=FFMIN(size2,sizeof(priv->avih));
      if( mp_msg_test(MSGT_HEADER,MSGL_V) ) print_avih(&(priv->avih),MSGL_V); // else print_avih_flags(&avih,MSGL_V);
      break;
    case ckidSTREAMHEADER: {      // read 'strh'
      ...
      priv->last_fccType=h.fccType;
      priv->last_fccHandler=h.fccHandler;
    ...
    case ckidSTREAMFORMAT: {      // read 'strf'
      if(priv->last_fccType==streamtypeVIDEO){
...
//        if(demuxer->video->id==-1) demuxer->video->id=stream_id;
	...
	case mmioFOURCC('D', 'I', 'V', '2'):
      ...
      if(priv->last_fccType==streamtypeAUDIO){
...
//        sh_audio->wf=malloc(chunksize); memset(sh_audio->wf,0,chunksize);
	...
	if (sh_audio->format == 1 &&
	    priv->last_fccHandler == mmioFOURCC('A', 'x', 'a', 'n'))
	    sh_audio->format = priv->last_fccHandler;
...
//        if(demuxer->audio->id==-1) demuxer->audio->id=stream_id;
    ...
    if(index_mode && !priv->isodml){
      ...
      mp_msg(MSGT_HEADER,MSGL_V,MSGTR_MPDEMUX_AVIHDR_ReadingIndexBlockChunksForFrames,
        priv->idx_size,priv->avih.dwTotalFrames, (int64_t)stream_tell(demuxer->stream));
...
//      printf("\nindex to %p !!!!! (priv=%p)\n",priv->idx,priv);
 ...
 if ((priv->suidx_size > 0 && priv->idx_size == 0) || (priv->idx_size == 2 && priv->suidx_size == 2)) {
    /*
...
if (priv->isodml && (index_mode==-1 || index_mode==0 || index_mode==1)) {
	...
	for (j=0; j<cx->nEntriesInUse; j++) {
	    ...
	    ret1 = stream_seek(demuxer->stream, (loff_t)cx->aIndex[j].qwOffset);
...
//      priv->idx_size+=32;
    ...
    // update status line:
    { // static loff_t lastpos;
      loff_t pos;
      loff_t len=demuxer->movi_end-demuxer->movi_start;
      if(len){
