...
//Packs are freed.
static float read_first_mpeg_pts_at_position(demuxer_t* demuxer, loff_t stream_pos)
{
...
static float read_final_mpeg_pts_at_position(demuxer_t* demuxer, loff_t stream_pos)
{
  stream_t *s = demuxer->stream;
  mpg_demuxer_t *mpg_d = demuxer->priv;
  float pts = -1.0; //the pts to return;
  ...
  //clean up from searching of final pts;
  demux_flush(demuxer);
  return pts;
}
...
static unsigned long long read_mpeg_timestamp(stream_t *s,int c,demuxer_t* demuxer){
  unsigned int d,e;
  unsigned long long pts;

  if (demuxer->glinfo == NULL){
    demuxer->glinfo = calloc(1,sizeof(mpg_glinfo_t));
  }
  mpg_glinfo_t *mp_glinfo = (mpg_glinfo_t *)demuxer->glinfo;

  d=stream_read_word(s);
  e=stream_read_word(s);
  if( ((c&1)!=1) || ((d&1)!=1) || ((e&1)!=1) ){
    ++(mp_glinfo->mpeg_pts_error);
...
static int demux_mpg_read_packet(demuxer_t *demux,int id){
  ...
  if (demux->glinfo == NULL){
    demux->glinfo = calloc(1,sizeof(mpg_glinfo_t));
  }
  mpg_glinfo_t *mp_glinfo = (mpg_glinfo_t *)demux->glinfo;
...
//    printf("  STD_size=%d",d);
  ...
  if((c>>4)==2){
    pts=read_mpeg_timestamp(demux->stream,c,demux);
    set_pts=1;
    len-=4;
  } else
  if((c>>4)==3){
    ...
    dts=read_mpeg_timestamp(demux->stream,c,demux);
    len-=4+1+4;
  } else
  if((c>>6)==2){
    int pts_flags;
...
//        printf("  \n");
  ...
  if(mp_glinfo->mpeg_pts_error) mp_msg(MSGT_DEMUX,MSGL_V,"  {PTS_err:%d}  \n",mp_glinfo->mpeg_pts_error);
...
//    printf("packet start = 0x%X  \n",stream_tell(demux->stream)-packet_start_pos);
    ...
    len = l;
    if(set_pts)      dp->pts=pts/90000.0f;
...
static void clear_stats(demuxer_t *demux)
{
  if (demux->glinfo == NULL){
    demux->glinfo = calloc(1,sizeof(mpg_glinfo_t));
  }
  mpg_glinfo_t *mp_glinfo = (mpg_glinfo_t *)demux->glinfo;
  mp_glinfo->num_elementary_packets100=0;
  mp_glinfo->num_elementary_packets101=0;
  mp_glinfo->num_elementary_packets1B6=0;
  mp_glinfo->num_elementary_packets12x=0;
  mp_glinfo->num_elementary_packetsPES=0;
  mp_glinfo->num_mpeg12_startcode=0;
  mp_glinfo->num_h264_slice=0; //combined slice
  mp_glinfo->num_h264_dpa=0; //DPA Slice
  mp_glinfo->num_h264_dpb=0; //DPB Slice
  mp_glinfo->num_h264_dpc=0; //DPC Slice
  mp_glinfo->num_h264_idr=0; //IDR Slice
  mp_glinfo->num_h264_sps=0;
  mp_glinfo->num_h264_pps=0;
  mp_glinfo->num_mp3audio_packets=0;
}

//assumes demuxer->synced < 2
static inline void update_stats(int head, demuxer_t *demux)
{
  if (demux->glinfo == NULL){
    demux->glinfo = calloc(1,sizeof(mpg_glinfo_t));
  }
  ...
  if(head==0x1B6) ++(mp_glinfo->num_elementary_packets1B6);
  else if(head==0x1B3 || head==0x1B8) ++(mp_glinfo->num_mpeg12_startcode);
  else if(head==0x100) ++(mp_glinfo->num_elementary_packets100);
  else if(head==0x101) ++(mp_glinfo->num_elementary_packets101);
  else if(head==0x1BD || (0x1C0<=head && head<=0x1EF))
    (mp_glinfo->num_elementary_packetsPES)++;
  else if(head>=0x120 && head<=0x12F) ++(mp_glinfo->num_elementary_packets12x);
  if(head>=0x100 && head<0x1B0)
  {
    if((head&~0x60) == 0x101) ++(mp_glinfo->num_h264_slice);
    else if((head&~0x60) == 0x102) ++(mp_glinfo->num_h264_dpa);
    else if((head&~0x60) == 0x103) ++(mp_glinfo->num_h264_dpb);
    else if((head&~0x60) == 0x104) ++(mp_glinfo->num_h264_dpc);
    else if((head&~0x60) == 0x105 && head != 0x105) ++(mp_glinfo->num_h264_idr);
    else if((head&~0x60) == 0x107 && head != 0x107) ++(mp_glinfo->num_h264_sps);
    else if((head&~0x60) == 0x108 && head != 0x108) ++(mp_glinfo->num_h264_pps);
  }
}
...
static int demux_mpg_probe(demuxer_t *demuxer) {
  ...
  if(demux_mpg_open(demuxer))
    ...
    mp_msg(MSGT_DEMUX,MSGL_V,"MPEG packet stats: p100: %d  p101: %d p1B6: %d p12x: %d sli: %d a: %d b: %d c: %d idr: %d sps: %d pps: %d PES: %d  MP3: %d, synced: %d\n",
     mp_glinfo->num_elementary_packets100,mp_glinfo->num_elementary_packets101,
     mp_glinfo->num_elementary_packets1B6,mp_glinfo->num_elementary_packets12x,
     mp_glinfo->num_h264_slice, mp_glinfo->num_h264_dpa,
     mp_glinfo->num_h264_dpb, mp_glinfo->num_h264_dpc=0,
     mp_glinfo->num_h264_idr, mp_glinfo->num_h264_sps=0,
     mp_glinfo->num_h264_pps,
     mp_glinfo->num_elementary_packetsPES,mp_glinfo->num_mp3audio_packets, demuxer->synced);

     //MPEG packet stats: p100: 458  p101: 458  PES: 0  MP3: 1103  (.m2v)
     if(mp_glinfo->num_mp3audio_packets>50 && mp_glinfo->num_mp3audio_packets>2*mp_glinfo->num_elementary_packets100
        && abs(mp_glinfo->num_elementary_packets100-mp_glinfo->num_elementary_packets101)>2)
       return file_format;

      // some hack to get meaningfull error messages to our unhappy users:
      if(mp_glinfo->num_mpeg12_startcode>=2 && mp_glinfo->num_elementary_packets100>=2 && mp_glinfo->num_elementary_packets101>=2 &&
         abs(mp_glinfo->num_elementary_packets101+8-mp_glinfo->num_elementary_packets100)<16) {
         if(mp_glinfo->num_elementary_packetsPES>=4 && mp_glinfo->num_elementary_packetsPES>=mp_glinfo->num_elementary_packets100-4) {
           return file_format;
         }
        ...
        if(mp_glinfo->num_elementary_packets1B6>3 && mp_glinfo->num_elementary_packets12x>=1 &&
           mp_glinfo->num_elementary_packetsPES==0 && mp_glinfo->num_elementary_packets100<=mp_glinfo->num_elementary_packets12x &&
           demuxer->synced<2) {
             file_format=DEMUXER_TYPE_MPEG4_ES;
        } else
         // fuzzy h264-es detection. do NOT enable without heavy testing of mpeg formats detection!
        if((mp_glinfo->num_h264_slice>3 || (mp_glinfo->num_h264_dpa>3 && mp_glinfo->num_h264_dpb>3 && mp_glinfo->num_h264_dpc>3)) &&
          /* FIXME num_h264_sps>=1 && */ mp_glinfo->num_h264_pps>=1 && mp_glinfo->num_h264_idr>=1 &&
          mp_glinfo->num_elementary_packets1B6==0 && mp_glinfo->num_elementary_packetsPES==0 &&
          demuxer->synced<2) {
...
do{
  ...
  if((head&0xFFFFFF00)!=0x100){
   ...
   while(1){
	...
	if(mp_glinfo->num_mp3audio_packets>100){
	  demux->stream->eof=1;
	  break;
	}
	++(mp_glinfo->num_mp3audio_packets);
      }
...
//    if(head==0x1BD || (head>=0x1C0 && head<=0x1EF)) demux->synced=3; // PES?
  ...
  if(demux->synced==1){
    if(head==0x1BB || head==0x1BD || (head>=0x1C0 && head<=0x1EF)){
      ...
      mp_glinfo->num_elementary_packets100=0; // requires for re-sync!
      mp_glinfo->num_elementary_packets101=0; // requires for re-sync!
    } else demux->synced=0;
  } // else
  if(demux->synced>=2){
    ...
    if(((mp_glinfo->num_elementary_packets100>50 && mp_glinfo->num_elementary_packets101>50) ||
        (mp_glinfo->num_elementary_packetsPES>50)) && skipped>4000000){
        mp_msg(MSGT_DEMUX,MSGL_V,"sync_mpeg_ps: seems to be ES/PES stream...\n");
        demux->stream->eof=1;
        break;
    }
    if(mp_glinfo->num_mp3audio_packets>100 && mp_glinfo->num_elementary_packets100<10){
                           ...
                           float audio_delay, int flags)
{
    ...
    int precision = 1;
    float oldpts = 0;
    loff_t oldpos = demuxer->filepos;
    float newpts = 0;
    float firstpts = 0;
    loff_t newpos = (flags & SEEK_ABSOLUTE) ? demuxer->movi_start : oldpos;
    if(mpg_d){
      oldpts = mpg_d->last_pts;
      firstpts = mpg_d->first_pts;
    }
    newpts = (flags & SEEK_ABSOLUTE) ? firstpts : oldpts;
  //================= seek in MPEG ==========================
  //calculate the pts to seek to
    ...
    while (1) {
        ...
        // re-sync video:
	sh_video->videobuf_code_len=0; // reset ES stream buffer
	...
	while(1){
	  ...
	  if(!sh_video){
	    break;
	      ...
	      int pos = sh_video->videobuf_len;
	      if(!read_video_packet(d_video)) break; // EOF
	      if((sh_video->videobuffer[pos+4] & 0x3F) == 0) break;	//I-frame
	    }
	...
	if (!precision || abs(newpts - mpg_d->last_pts) < 0.5 || mpg_d->last_pts == oldpts) {
	  break;
	}
...
static demuxer_t* demux_mpg_ps_open(demuxer_t* demuxer)
{
    ...
    if(!sh_video->format && ps_probe > 0) {
        int head;
        loff_t pos = stream_tell(demuxer->stream);

        clear_stats(demuxer);
        do {
            head=sync_video_packet(demuxer->video);
            if(!head) break;
            update_stats(head,demuxer);
        ...
        mp_msg(MSGT_DEMUX,MSGL_INFO,"MPEG packet stats: p100: %d  p101: %d p1B6: %d p12x: %d sli: %d a: %d b: %d c: %d idr: %d sps: %d pps: %d\n",
            mp_glinfo->num_elementary_packets100, mp_glinfo->num_elementary_packets101,
            mp_glinfo->num_elementary_packets1B6, mp_glinfo->num_elementary_packets12x,
            mp_glinfo->num_h264_slice, mp_glinfo->num_h264_dpa, mp_glinfo->num_h264_dpb, mp_glinfo->num_h264_dpc,
            mp_glinfo->num_h264_idr, mp_glinfo->num_h264_sps, mp_glinfo->num_h264_pps);

        if(mp_glinfo->num_elementary_packets1B6>3 && mp_glinfo->num_elementary_packets12x>=1 &&
            mp_glinfo->num_elementary_packets100<=mp_glinfo->num_elementary_packets12x)
            sh_video->format = 0x10000004;
        else if((mp_glinfo->num_h264_slice>3 || (mp_glinfo->num_h264_dpa>3 && mp_glinfo->num_h264_dpb>3 && mp_glinfo->num_h264_dpc>3)) &&
            mp_glinfo->num_h264_sps>=1 && mp_glinfo->num_h264_pps>=1 && mp_glinfo->num_h264_idr>=1 &&
            mp_glinfo->num_elementary_packets1B6==0)
