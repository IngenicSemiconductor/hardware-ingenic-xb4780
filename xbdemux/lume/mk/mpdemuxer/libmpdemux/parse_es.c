...
int sync_video_packet(demux_stream_t *ds){
	int *videobuf_code_len = &(((sh_video_t *)ds->sh)->videobuf_code_len);
	int *next_nal = &(((sh_video_t *)ds->sh)->next_nal);
...
int packet_start;
  ...
  unsigned char* videobuffer = ((sh_video_t *)ds->sh)->videobuffer;
  int *next_nal = &(((sh_video_t *)ds->sh)->next_nal);
  int *pvideobuf_len = &((sh_video_t *)ds->sh)->videobuf_len;
  int *pvideobuf_code_len = &((sh_video_t *)ds->sh)->videobuf_code_len;

  if (VIDEOBUFFER_SIZE - *pvideobuf_len < 5)
    return 0;
  // SYNC STREAM
//  if(!sync_video_packet(ds)) return 0; // cannot sync (EOF)

  // COPY STARTCODE:
  packet_start=*pvideobuf_len;
  videobuffer[*pvideobuf_len+0]=0;
  videobuffer[*pvideobuf_len+1]=0;
  videobuffer[*pvideobuf_len+2]=1;
  videobuffer[*pvideobuf_len+3]=*next_nal;
  *pvideobuf_len+=4;

  // READ PACKET:
  res = demux_pattern_3(ds, &videobuffer[*pvideobuf_len],
          VIDEOBUFFER_SIZE - *pvideobuf_len, &read, 0x100);
  *pvideobuf_len += read;
  if (!res)
    goto eof_out;
  ...
  *pvideobuf_len-=3;
  ...
  mp_dbg(MSGT_PARSEES,MSGL_DBG2,"videobuf: packet 0x1%02X  len=%d  (total=%d)\n",videobuffer[packet_start+3],*pvideobuf_len-packet_start,*pvideobuf_len);

  // Save next packet code:
  *next_nal = demux_getc(ds);
  if (*next_nal < 0)
    goto eof_out;
  *pvideobuf_code_len=4;

  return *pvideobuf_len-packet_start;

eof_out:
  *next_nal = -1;
  *pvideobuf_code_len = 0;
  return *pvideobuf_len - packet_start;
}
