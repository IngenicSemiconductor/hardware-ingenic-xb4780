...
static int demux_pmp_fill_buffer(demuxer_t *demuxer, demux_stream_t *ds)
{
    ...
    /* Video */
    if (ds == demuxer->video) {
      //LOGE("p->current_video_frame:%d num_of_frames:%d", p->current_video_frame, p->stream.header.video.number_of_frames);
    ...
    /* Audio */
    if (ds == demuxer->audio) {
      //LOGE("current_audio_frame:%d, number_of_frame:%d %d", p->current_audio_frame, p->stream.header.video.number_of_frames, p->stream.header.audio.number_of_streams);
        ...
        if(p->stream.header.audio.format == 1) {
            ...
            if(p->pmp_aid > 0) {
                audiopos += len;
                int j = 1;
                while(j < header.number_of_audio_frames*p->pmp_aid) {
                    stream_read(s, &len, 4);
                    audiopos += len;
                    ++j;
                }
            ...
            while(i < header.number_of_audio_frames) {
                stream_seek(s, framepos + 1 + 4 + 4 + 4 + 4*i + 4*p->pmp_aid*header.number_of_audio_frames);
...
static int demux_pmp_control(demuxer_t *demuxer, int cmd, void *arg)
{
    ...
    case DEMUXER_CTRL_SWITCH_AUDIO:
        ...
        if(id < 0) {
	  ++(pmp_d->pmp_aid);
            pmp_d->pmp_aid %= pmp_d->stream.header.audio.number_of_streams;
        } else {
            pmp_d->pmp_aid = id % pmp_d->stream.header.audio.number_of_streams;
        }
        demuxer->audio->id = pmp_d->pmp_aid;
