...
mp_mpeg_header_t *picture ;
 ...
 if(!sh_video->videopicture) {
   sh_video->videopicture = malloc(sizeof (mp_mpeg_header_t));
 }
 picture = sh_video->videopicture;
 // Determine image properties:
switch(video_codec){
 case VIDEO_OTHER: {
#if defined(JZ4740_IPU) && defined(JZC_BS_HW_OPT)
   if(!sh_video->videobuffer) {
     sh_video->videobuffer=(char*)memalign(8,VIDEOBUFFER_SIZE + FF_INPUT_BUFFER_PADDING_SIZE);
     if (sh_video->videobuffer) memset(sh_video->videobuffer+VIDEOBUFFER_SIZE, 0, FF_INPUT_BUFFER_PADDING_SIZE);
     else {
       mp_msg(NULL,NULL," !! alloc videobuffer failed !! \n");
       return 0;
     }
   }
#endif // defined(JZ4740_IPU) && defined(JZC_BS_HW_OPT)
 ...
 case VIDEO_MPEG4: {
   ...
   sh_video->videobuf_len=0; sh_video->videobuf_code_len=0;
   ...
   if(!sh_video->videobuffer) {
     sh_video->videobuffer=(char*)memalign(8,VIDEOBUFFER_SIZE + MP_INPUT_BUFFER_PADDING_SIZE);
     if (sh_video->videobuffer) memset(sh_video->videobuffer+VIDEOBUFFER_SIZE, 0, MP_INPUT_BUFFER_PADDING_SIZE);
     else {
   ...
   pos = sh_video->videobuf_len+4;
   if(!read_video_packet(d_video)){
     mp_msg(MSGT_DECVIDEO,MSGL_ERR,"Can't read Video Object Layer Header\n");
     return 0;
   }
   mp4_header_process_vol(picture, &(sh_video->videobuffer[pos]));
 ...
 mp4_init:
   ...
   mp4_header_process_vop(picture, &(sh_video->videobuffer[pos]));
 ...
 case VIDEO_H264: {
   ...
   h264_parse_sps(picture, &(sh_video->videobuffer[pos]), sh_video->videobuf_len - pos);
   sh_video->disp_w=picture->display_picture_width;
   sh_video->disp_h=picture->display_picture_height;
...
mpeg_header_parser:
   ...
   sh_video->videobuf_len=0; sh_video->videobuf_code_len=0;
   sh_video->telecine=0; sh_video->telecine_cnt=-2.5;
   ...
   if(mp_header_process_sequence_header (picture, &sh_video->videobuffer[4])) {
   ...
   if(sync_video_packet(d_video)==0x1B5){ // next packet is seq. ext.
    ...
    if(mp_header_process_extension (picture, &sh_video->videobuffer[pos+4])) {
 ...
 case VIDEO_VC1: {
   ...
   if(mp_vc1_decode_sequence_header(picture, &sh_video->videobuffer[4], sh_video->videobuf_len-4)) {
     sh_video->bih = calloc(1, sizeof(BITMAPINFOHEADER) + sh_video->videobuf_len);
     if(sh_video->bih == NULL) {
       mp_msg(MSGT_DECVIDEO,MSGL_ERR,"Couldn't alloc %d bytes for VC-1 extradata!\n", sizeof(BITMAPINFOHEADER) + sh_video->videobuf_len);
       return 0;
     }
     sh_video->bih->biSize= sizeof(BITMAPINFOHEADER) + sh_video->videobuf_len;
     memcpy(sh_video->bih + 1, sh_video->videobuffer, sh_video->videobuf_len);
     ...
     mp_msg(MSGT_DECVIDEO,MSGL_INFO,"VIDEO:  VC-1  %dx%d, %5.3f fps, header len: %d\n",
       sh_video->disp_w, sh_video->disp_h, sh_video->fps, sh_video->videobuf_len);
   }
  break;
 }
} // switch(file_format)
...
int video_read_frame(sh_video_t* sh_video,float* frame_time_ptr,unsigned char** start,int force_fps){
    ...
    int in_size=0;
    video_codec_t video_codec = find_video_codec(sh_video);
    mp_mpeg_header_t *picture = sh_video->videopicture;    
    *start=NULL;
    if(video_codec == VIDEO_MPEG12){
        int in_frame=0;
        //float newfps;
        //sh_video->videobuf_len=0;
        while(sh_video->videobuf_len<VIDEOBUFFER_SIZE-MAX_VIDEO_PACKET_SIZE){
          int i=sync_video_packet(d_video);
          //void* buffer=&sh_video->videobuffer[sh_video->videobuf_len+4];
          int start=sh_video->videobuf_len+4;
          if(in_frame){
          ...
          // process headers:
          switch(i){
            case 0x1B3: mp_header_process_sequence_header (picture, &sh_video->videobuffer[start]);break;
            case 0x1B5: mp_header_process_extension (picture, &sh_video->videobuffer[start]);break;
            case 0x1B2: process_userdata (&sh_video->videobuffer[start], sh_video->videobuf_len-start);break;
            case 0x100: picture_coding_type=(sh_video->videobuffer[start+1] >> 3) & 7;break;
          }
        }
        fps = picture->fps * picture->frame_rate_extension_n / picture->frame_rate_extension_d;

        *start=sh_video->videobuffer; in_size=sh_video->videobuf_len;

    // get mpeg fps:
    if(sh_video->fps!=fps) if(!force_fps && !sh_video->telecine){
    ...
    sh_video->telecine_cnt*=0.9; // drift out error
    sh_video->telecine_cnt+=frame_time-5.0/4.0;
    mp_msg(MSGT_DECVIDEO,MSGL_DBG2,"\r sh_video->telecine = %3.1f  %5.3f     \n",frame_time,sh_video->telecine_cnt);

    if(sh_video->telecine){
        frame_time=1;
        if(sh_video->telecine_cnt<-1.5 || sh_video->telecine_cnt>1.5){
            mp_msg(MSGT_DECVIDEO,MSGL_INFO,MSGTR_LeaveTelecineMode);
            sh_video->telecine=0;
        }
    } else
        if(sh_video->telecine_cnt>-0.5 && sh_video->telecine_cnt<0.5 && !force_fps){
      ...
      //printf("enter: else if(video_codec == VIDEO_H264)\n");
        int in_picture = 0;
        while(sh_video->videobuf_len<VIDEOBUFFER_SIZE-MAX_VIDEO_PACKET_SIZE){
          int i=sync_video_packet(d_video);
          int pos = sh_video->videobuf_len+4;
          if(!i) return -1;
          if(!read_video_packet(d_video)) return -1; // EOF
	    ...
	    //printf("\nenter: if (in_picture)\n");
	      ...
	      //printf("enter: if(i == 0x101 || i == 0x102 || i == 0x105)\n");
		...
		//printf("enter: if (demux_peekc(d_video) & 0x80)\n");
                break;
	      }
            }
          }
       ...
       *start=sh_video->videobuffer;
       in_size=sh_video->videobuf_len;
       sh_video->videobuf_len=0;
  } else {
...
#if defined(JZ4740_IPU) && defined(JZC_BS_HW_OPT)
    fast_memcpy(sh_video->videobuffer,*start,in_size);
    *start = sh_video->videobuffer;
#endif
    //printf("enter: video_codec == VIDEO_OTHER\n");
  }
