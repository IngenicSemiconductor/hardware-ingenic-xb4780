...
typedef struct mp3_hdr {
  loff_t frame_pos; // start of first frame in this "chain" of headers
  loff_t next_frame_pos; // here we expect the next header with same parameters
  int mp3_chans;
  int mp3_freq;
  int mpa_spf;
  int mpa_layer;
 ...
 */
static mp3_hdr_t *add_mp3_hdr(mp3_hdr_t **list, loff_t st_pos,
      ...
      if (stream_read (s, comments, blk_len) == blk_len)
      {
        ...
        for (; cn < comment_list_len; cn++)
        {
                   ...
                   && (length - 12 > 0))
          {
            char buf[31];
            buf[30] = '\0';
            snprintf (buf, 31, "%d", atoi (comment + 12));
...
static int demux_audio_open(demuxer_t* demuxer) {
  ...
  int frmt = 0, n = 0, step;
  loff_t st_pos = 0, next_frame_pos = 0;
  ...
  while(n < 30000 && !s->eof) {
	...
	break;
      ...
      if(hdr[0] == 'C' || hdr[1] == 'D' || hdr[2] == 'X'  || hdr[3] == 'A' )//"CDXA" is video so check fail 20111020 hpwang
	return 0;
...
/*find duration VBR */
  if(frmt == MP3){
      ...
      int stereo    = ( (((newhead>>6)&0x3)) == 3) ? 1 : 2;
      ...
      stream_seek(s,mp3_found->frame_pos+ssize+4);

      /* XING info start */
      /*  4 XING - 4 flags - 4 frames - 4 bytes - 100 toc */
      stream_read(s, pBuf, 16); 
      if ((memcmp(pBuf, STR_XING_HEADER_IDENTIFIER,VBR_HEADER_SIZE) == 0)/*||
    	   (memcmp(pBuf, STR_INFO_HEADER_IDENTIFIER,VBR_HEADER_SIZE) == 0)*/)
      {
		  pBuf += 4;
		  int32_t head_flags = (*(pBuf)<<24)|(*(pBuf+1)<<16)|(*(pBuf+2)<<8)|(*(pBuf+3));
		  if (head_flags & FRAMES_FLAG)
		  {   
			  pBuf += 4;
			  int mp3frames = (*(pBuf)<<24)|(*(pBuf+1)<<16)|(*(pBuf+2)<<8)|(*(pBuf+3));
			  //LOGE("mp3frame = %d,mp3_found->mpa_spf = %d,mp3_found->mp3_freq = %d",mp3frames,mp3_found->mpa_spf,mp3_found->mp3_freq);
			  mp3duration = (double)(mp3frames * mp3_found->mpa_spf)/mp3_found->mp3_freq;
		  }
      }else{
		  /*CBR*/
		  //EL("CBR file");
	          mp3duration = get_mp3_duration(demuxer);
	  }     
#endif
      mp3duration = get_mp3_duration(demuxer);
  }
...
//    printf("wav: %X .. %X\n",(int)demuxer->movi_start,(int)demuxer->movi_end);
  ...
  if(mp3duration > 0.0) priv->mp3duration = mp3duration;
  else priv->mp3duration = 0.0;
...
static int demux_audio_fill_buffer(demuxer_t *demux, demux_stream_t *ds) {
  ...
  case MP3 :
    while(1) {
      ...
      int len = 0;
    re_read:
      len = stream_read(s,hdr,4);
      if (s->eof)
        return 0;
      //l = mp_decode_mp3_header(hdr);
      int srate = 0;
      l = mp_get_mp3_header(hdr, NULL, &srate, NULL, NULL, NULL);
#if 1 // hpwang 2011-10-24 skip stream when samplerate is change. may be some error...
      if (demux->last_sample == 0)
	demux->last_sample = srate;
      else{
	if (demux->last_sample != srate){
	  stream_skip(s, len);
	  goto re_read;
	}
      }
...
static void high_res_mp3_seek(demuxer_t *demuxer,float time) {
  ...
  while(nf > 0) {
    if(stream_read(demuxer->stream,hdr,4)!=4){
      break;
    }
...
//  LOGE("rel_seek_secs = %f",rel_seek_secs);
...
static int demux_audio_control(demuxer_t *demuxer,int cmd, void *arg){
    ...
    double audio_length = sh_audio->i_bps ? (double)((double)(demuxer->movi_end-demuxer->movi_start) / (double)(sh_audio->i_bps)) : 0.0;    
    da_priv_t* priv = demuxer->priv;

    switch(cmd) {
	case DEMUXER_CTRL_GET_TIME_LENGTH:
        //LOGE("=priv->mp3duration = %f",priv->mp3duration);
	    if (priv->mp3duration > 0.0) {
		*((double *)arg)=priv->mp3duration;
