...
typedef struct mkv_demuxer {
    ...
    int index_mode;  // move this from global variable of demux_avi.c to here
} mkv_demuxer_t;
                            ...
                            uint8_t **dest, uint32_t *size, uint32_t type)
{
    ...
    *dest = src;
	...
	if (track->encodings[i].comp_algo == 3) { 
	 modified =  track->encodings[i].comp_settings_len; 
	}
    }
...
static int demux_mkv_read_cues(demuxer_t *demuxer)
{
    ...
    mkv_d->parsed_cues = realloc(mkv_d->parsed_cues,
                                 (mkv_d->parsed_cues_num + 1) * sizeof(loff_t));
        ...
        *str = '\0';
        ...
        case MATROSKA_TRACK_AUDIO:
            ...
            if (mkv_d->tracks[i]->name)
                mp_msg(MSGT_IDENTIFY, MSGL_INFO, "ID_AID_%d_NAME=%s\n", aid,
                       mkv_d->tracks[i]->name);
            snprintf(str, 32, "-aid %u, -alang %.5s", aid++,
        ...
        case MATROSKA_TRACK_SUBTITLE:
            ...
            mp_msg(MSGT_IDENTIFY, MSGL_INFO, "ID_SID_%d_LANG=%s\n", sid,
                   mkv_d->tracks[i]->language);
            snprintf(str, 32, "-sid %u, -slang %.5s", sid++,
                    mkv_d->tracks[i]->language);
            break;
        }
        if (mkv_d->tracks[i]->name)
                                ...
                                int aid)
{
    ...
    if (!sh_a){
     ...
     sh_a->need_parsing = 0; //add by gysun
...
static int demux_mkv_open(demuxer_t *demuxer)
{
    ...
    mkv_d->parsed_cues = malloc(sizeof(loff_t));
    mkv_d->parsed_seekhead = malloc(sizeof(loff_t));
	...
	*/
    ...
    if(mkv_d->indexes == NULL && mkv_d->index_mode < 0)
      {
	demuxer->thumb_nseekable = 1;
      }
      
    if (s->end_pos == 0 || (mkv_d->indexes == NULL && mkv_d->index_mode < 0)){
        demuxer->seekable = s->end_pos?1:0;
    }
    else {
    ...
    demuxer->audio_info = malloc(100 * mkv_d->num_tracks);
    int j = 0;
    for (i = 0; i < mkv_d->num_tracks; i++){
      if (mkv_d->tracks[i]->type == MATROSKA_TRACK_AUDIO) {
	char *info = (char *)malloc(100);
	strcpy(info,mkv_d->tracks[i]->language);
	if(mkv_d->tracks[i]->name){
	  sprintf(info, "%s[%s]", info, mkv_d->tracks[i]->name);
	}
	sprintf(info, "%s--%d", info, mkv_d->tracks[i]->tnum);
    ...
    j = 0;
    demuxer->sub_info = malloc(100 * mkv_d->num_tracks);
    for (i = 0; i < mkv_d->num_tracks; i++){
                                 ...
                                 int block_bref, int block_fref)
{
    mkv_demuxer_t *mkv_d = (mkv_demuxer_t *) demuxer->priv;
    demux_packet_t *dp;
    int modified = demux_mkv_decode(track, buffer, &buffer, &size, 1);
    if (buffer) {
      dp = new_demux_packet(size+modified);
      if (modified)
	memcpy (dp->buffer, track->encodings->comp_settings, modified);
      memcpy(dp->buffer+modified, buffer, size);
    }
    //dp = new_demux_packet(size);
    //memcpy(dp->buffer, buffer, size);
    dp->pos = demuxer->filepos;
...
static int demux_mkv_fill_buffer(demuxer_t *demuxer, demux_stream_t *ds)
{
    ...
    if(demuxer->a_changed_id != -2){
      if(demuxer->a_changed_id!=demuxer->audio->id && demuxer->a_streams[demuxer->a_changed_id]){
    ...
    if(demuxer->s_changed_id){
      if(demuxer->s_changed_id != demuxer->sub->id){
	demuxer->sub->id = demuxer->s_changed_id;  
	demuxer->sub->sh = demuxer->s_streams[demuxer->sub->id];
      }
                           ...
                           float audio_delay, int flags)
{
    ...
    if (!(flags & SEEK_FACTOR)) {       /* time in secs */
	    ...
	    if (mkv_d->num_indexes > 0 && mkv_d->skip_to_timecode > mkv_d->indexes[mkv_d->num_indexes - 1].timecode)
	      mkv_d->skip_to_timecode = mkv_d->indexes[mkv_d->num_indexes - 1].timecode;
	}
