...
static int mov_read_stss(MOVContext *c, ByteIOContext *pb, MOVAtom atom)
{
    ...
    sc->stss_val = av_malloc(entries * sizeof(int));

    for(i=0; i<entries; i++) {
        sc->keyframes[i] = get_be32(pb) - 1;
	sc->stss_val[i] = sc->keyframes[i];
    }
    return 0;
}
...
static int mov_read_tkhd(MOVContext *c, ByteIOContext *pb, MOVAtom atom)
{
    ...
    */
    ...
    if (display_matrix[0][0] == kFixedOne && display_matrix[0][1] == 0 
	&& display_matrix[1][0] == 0 && display_matrix[1][1] == kFixedOne) {
        // Identity, no rotation
        st->rotation_degrees = 0;
    } else if (display_matrix[0][0] == 0 && display_matrix[0][1] == kFixedOne 
	       && display_matrix[1][0] == -kFixedOne && display_matrix[1][1] == 0) {
        st->rotation_degrees = 90;
    } else if (display_matrix[0][0] == 0 && display_matrix[0][1] == -kFixedOne 
	       && display_matrix[1][0] == kFixedOne && display_matrix[1][1] == 0) {
        st->rotation_degrees = 270;
    } else if (display_matrix[0][0] == -kFixedOne && display_matrix[0][1] == 0 
	       && display_matrix[1][0] == 0 && display_matrix[1][1] == -kFixedOne) {
        st->rotation_degrees = 180;
    } else {
        st->rotation_degrees = 0;
    }
...
static int mov_read_cmov(MOVContext *c, ByteIOContext *pb, MOVAtom atom)
{
    ...
    if(uncompress (moov_data, (unsigned long *) &moov_len, (const uint8_t *)cmov_data, cmov_len) != 0)
...
static int mov_seek_stream(AVFormatContext *s, AVStream *st, int64_t timestamp, int flags)
{
    ...
    if (timestamp == 0){
      flags ^= AVSEEK_FLAG_BACKWARD; //hpwang 2012-04-09 only for qq video when play a mp4 url
    }
    ...
    for (i = 1; i < sc->keyframe_count; i++){
      if (sample < sc->stss_val[i]){
	sample = sc->stss_val[i-1];
	break;
      }
    }
