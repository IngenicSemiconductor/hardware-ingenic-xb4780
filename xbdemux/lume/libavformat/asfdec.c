...
//printf("key:%d stream:%d seq:%d offset:%d replic_size:%d\n", asf->packet_key_frame, asf->stream_index, asf->packet_seq, //asf->packet_frag_offset, asf->packet_replic_size);
	...
	asf->frag_timestamp = get_le32(pb);
	if(!asf->video_offset)
	  {
	    asf->video_start = asf->frag_timestamp;
	  }

	if(asf->video_start > 0x80000000)//MAX_PTS
	  asf->packet_frag_timestamp = asf->frag_timestamp;
	else
	  asf->packet_frag_timestamp = asf->frag_timestamp - asf->video_start; // read PTS
	asf->video_offset ++;
...
//            av_log(s, AV_LOG_DEBUG, "\n");
	...
	//LOGE("asf->packet_frag_timestamp = %lld",asf->packet_frag_timestamp);
        rsize++;
    ...
    for (;;) {
        ...
        if (asf->packet_replic_size == 1) {
            ...
            //asf->packet_frag_timestamp = asf->packet_time_start;
       	    asf->packet_frag_timestamp = asf->packet_time_start - asf->video_start;
...
#else//skip the top-level objects to search for the simple index in ASF files
#define avio_tell   url_ftell
#define avio_seek   url_fseek
#define ff_get_guid get_guid
#define avio_skip   url_fskip
#define ff_get_guid get_guid
#define ff_guidcmp  guidcmp
#define avio_rl64   get_le64
#define avio_rl32   get_le32
#define avio_rl16   get_le16
...
static void asf_build_simple_index(AVFormatContext *s, int stream_index)
{
...

    {
        ...
        for (i=0;i<ict;i++){
            ...
            index = av_add_index_entry(s->streams[stream_index], pos, index_pts, s->packet_size, 0, AVINDEX_KEYFRAME);
            last_pos=pos;
            }
        }
	if(index>0)
...
static int asf_read_seek(AVFormatContext *s, int stream_index, int64_t pts, int flags)
{
    ...
    if(asf->video_start > 8000)
      pts += asf->video_start + 4000;
    else
      pts += asf->video_start;
